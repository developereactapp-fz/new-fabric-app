...
const getTypeGroup = (t) => {
  if (["shirt", "tuxedo"].includes(t)) return ["shirt", "tuxedo"];
  if (["pant", "tuxedo-pant"].includes(t)) return ["pant", "tuxedo-pant"];
  if (["jacket", "tuxedo-jacket"].includes(t)) return ["jacket", "tuxedo-jacket"];
  if (["coat", "waistcoat"].includes(t)) return ["coat", "waistcoat"];
  return garmentTypes;
};
...
export default function StyleSelectPage({ type: propType }) {
  ...
  const nextType = () => {
    ...
    const currentIndex = group.indexOf(type);
    const nextIndex = (currentIndex + 1) % group.length;
    handleTypeChange(group[nextIndex]);
  };
  ...
  const prevType = () => {
    ...
    handleTypeChange(group[prevIndex]);
  };
